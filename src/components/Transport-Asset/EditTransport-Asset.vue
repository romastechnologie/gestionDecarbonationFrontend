<template>
  <div class="card mb-25 border-0 rounded-0 bg-white add-user-card">
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
      <Form ref="transportForm" @submit="updateTransportAsset" :validation-schema="transportSchema">
        <div class="row">
          ...
          <div class="col-md-12">
            <div class="d-flex align-items-center">
              <button
                ...
                :disabled="isLoading"
              >
                {{ isLoading ? 'Modification...' : 'Modifier le transport' }}
              </button>
    ...
    onMounted(async () => {
      ...
      try {
        ...
        if (data && data.data) {
          ...
          const currentType = transport.type;

          if (transport.DateMiseProduction) {
        ...
        error('Impossible de charger les données du transport.');
      }
    });

    const updateTransportAsset = async (values: any) => {
      const id = route.params.id as string;
      ...
      try {
        const { data } = await ApiService.put(`/modifierTransport/${id}`, values);
        if (data.code === 200) {
